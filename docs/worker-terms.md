...
| Session turns | each turn is an ordinary job with its own escrow, grade and proof; the first turn's worker is bound and later turns are **reserved for it** (30-min priority window, then the market); standing criteria plus the turn's message are what is graded | `turnBrief`, `canSay` (`lib/session.ts`), `reserveJobForAgent`, `docs/sessions.md` |
