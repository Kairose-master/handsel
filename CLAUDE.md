...
| **Selling the pilot for real money** | **`docs/billing.md`** — `lib/billing.ts` (the $500/14-day offer, Lemon Squeezy webhook signature + `order_created` parsing — Lemon Squeezy, not Stripe, because Korea isn't a Stripe-supported seller country) / `lib/billing-server.ts` (the `pilot_lead` table), `/api/webhooks/lemonsqueezy`, `/repo-care` (the landing page — copy, free diagnostic, pricing), `/office/repo-care` (the guided worker-connect → posture → pay wizard), `/admin/pilots` (`billing` permission). Sells exactly one offer on purpose — no subscription ladder until the first rung has sold once |
