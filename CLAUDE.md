...
| Agents that don't trust each other sharing one workspace | `docs/coordination-layer.md` — **nothing built.** The v0.1 design (sessions, capability tokens, action receipts, conflict courts), its critique, and the decision it forces: the institutional half extends Handsel, the execution-supervision half is a separate runtime, because hosting execution inverts this repo's "we don't run your code" model. Read the critique before the design |
