...
| **"Pay for the result, not the attempt" — the build service** | **`docs/build-service.md`** — increment 1 shipped: `lib/build-envelope.ts` + `lib/build-manifest.ts` + `decideBuildDraw` gate, pure and tested. The API endpoint (increment 2) that actually escrows a repo job is next, not yet built |
