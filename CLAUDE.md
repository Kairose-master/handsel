...
| **Which real MCP servers actually work as workers** | **`docs/office-connectors.md`** — the four probed, the ones rejected and why, and the measurement behind `[mcp-query]`; `lib/verified-connectors.ts` is that record as the one-click catalog (test-pinned to the doc), now also on `/directory` (`components/verified-connector-grid.tsx`) as the curated tier between the real graded receipts and the passive ClawHub mirror — each card's live `ToolRecord` looked up by `verifiedConnectorToolId`, the same id a real agent's wiring earns |
...
| **The office pursuing a goal over time — plan, dispatch Claude Code on the owner's machine, checkpoint, verify, pay by policy, resume after a crash** | **`docs/office-sessions.md`** — `lib/office-session.ts` (state machine + reducer + invariants), `lib/office-session-loop.ts` (one heartbeat, pure), `lib/approval-policy.ts` (ALLOW / ALLOW_WITH_LOG / REQUIRE_OWNER / REQUIRE_REVIEWER / DENY, risk tiers E0–E4, the money gate), `lib/coding-harness.ts` (the `CodingHarness` contract; grant → Claude Code flags; brief on **stdin**), `lib/office-session-server.ts` (event log, worker protocol, commands), `lib/session-triggers.ts` (event-driven wakes: GitHub webhook names + the `http:` lane), `lib/session-tools.ts` (**talking outside the session over MCP**: a `consult` tool asked before each task, answer fenced into the brief and never evidence; a `notify` tool told one line — what `notifyText` builds and nothing else — on loop events), `/office/sessions` (en/ko), MCP `start_office_session` / `office_session_status` / `decide_session_approval` / `control_office_session`. Grants layer inward only (`narrowGrant`) and compile onto Codex/Gemini/OpenCode's own knobs (`harnessSessionArgv`); pause SIGSTOPs the harness; a settled internal task gets a signed proof; cloud/MCP/webhook agents take session tasks through the market's own `runAgentTask` + callback. Escrow is released only through `autoApprovePassedJob`; a policy release on real money needs `OFFICE_SESSION_ALLOW_REAL_MONEY=true` |
...
| **Selling the pilot and the office subscription for real money** | **`docs/billing.md`** — `lib/billing.ts` (the $500/14-day pilot offer + `OFFICE_SUBSCRIPTION_TIERS` (Starter/Growth/Studio, added 2026-09-05 overriding this doc's earlier "no subscription ladder" note), Lemon Squeezy webhook signature + `order_created`/`subscription_*` parsing — Lemon Squeezy, not Stripe, because Korea isn't a Stripe-supported seller country) / `lib/billing-server.ts` (`pilot_lead` + `office_subscription` tables), `/api/webhooks/lemonsqueezy`, `/repo-care` (the landing page — copy, free diagnostic, pricing), `/office/repo-care` (the guided worker-connect → posture → pay wizard), `/admin/pilots` (`billing` permission). A subscriber's plan limits are not wired to any account yet — `repoCareWithinTierLimits` is pure and unwired |
| **The first complete vertical — overnight repository care** | **`docs/repo-care.md`** — `lib/repo-care.ts` (pure triage: which issues an office works and which are left for a person, and why the skip list is a fixed table rather than a model's opinion), `startRepoCareSession`, `SessionTask.deliverPr` → the loop's `open_pr` → `openPrFromDiff`, the morning report — now with **real CI read back** onto the landed PR (`PR_CI_REPORTED`, `findRepoCareTaskForPr`/`recordPrCiVerdict`, the webhook's `maybeRecordRepoCareCi`), never settlement, its own "CI failed on a landed PR" heading ahead of "Landed". A configuration of the office-session runtime, not a second runtime |
| **What an office saved its owner, and what it may do without them** | `lib/office-metrics.ts` (pure — unattended rate, decisions asked for, time blocked waiting on a person, pass rate, model bill vs money moved; it refuses to claim hours saved), `PRESET_POLICIES` + `policyInWords` in `lib/approval-policy.ts` (three postures instead of a JSON editor) |
| **What this product actually sells, and the one thing to build next** | **`docs/positioning.md`** — §8 is the standing decision: an operations room priced monthly, first customer a 1-10 person agency, first vertical Repo Care. §9 (2026-09-04) is the sharpened sentence ("AI Agency Delivery OS", checked against real competitors, not a new decision) — the component split, the cold-start problem, the wedge (graded outcomes nobody else has), and why `/directory` is currently a mirror of someone else's registry |
| **Finding the two-week test's candidates** | **`docs/pilot-candidates.md`** — the sales-side ledger (ICP, falsifiable signal per row, status, scoreboard against §8's two-week test). A weekly Routine searches and appends rows; contacting a candidate is still the operator's own job — the ledger never sends anything |
