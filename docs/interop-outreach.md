...
| 6 | `daydreamsai/skills-market` [PR #58](https://github.com/daydreamsai/skills-market/pull/58) | jihadMo's bridge stub (hardcoded pass, nonexistent `handsel.dev`) — reviewed, not merged | **jihadMo responded positively** (2026-08-07): "share the implementation or open the follow-up PR — happy to fold it in or collaborate on the live grader integration", and pushed a new commit adding unit tests to the PR. `ExternalEvaluatorBridge` is now real and committed on our side (`lib/external-evaluator-bridge.ts`, `tests/external-evaluator-bridge.test.ts`, 10 tests) — it was only ever in scratchpad before this, so `docs/external-grading.md`'s "Live" claim is now actually true | Post the real implementation as a PR #58 comment (can't push directly — `daydreamsai` is out of session scope this session; comment drafted for the operator to paste) |
...
| 8 | nobulex (`nobulex.dev@gmail.com`) | Collaboration email: we consume registry records (FAIL_UNSAFE = routing data in a pay-on-pass market); our failed-job forensics feed their subject pipeline. One-directional, no ask | Draft v2 delivered to operator (failure-story + respect framing) | Operator sends. If reply: build our side only. If silence: consume the registry as a reader anyway — the email says so |
