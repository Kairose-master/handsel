...
| **The first complete vertical — overnight repository care** | **`docs/repo-care.md`** — `lib/repo-care.ts` (pure triage: which issues an office works and which are left for a person, and why the skip list is a fixed table rather than a model's opinion), `startRepoCareSession`, `SessionTask.deliverPr` → the loop's `open_pr` → `openPrFromDiff`, the morning report. A configuration of the office-session runtime, not a second runtime |
