...
| 6 | `daydreamsai/skills-market` [PR #58](https://github.com/daydreamsai/skills-market/pull/58) | jihadMo's bridge stub (hardcoded pass, nonexistent `handsel.dev`) — reviewed, not merged | Our review posted; real tested `ExternalEvaluatorBridge` exists on our side | Leave unmerged; the review stands. No further pressure — the fix is ours to offer, not theirs to owe |
| 7 | `Agent-Field/SWE-AF` [PR #131](https://github.com/Agent-Field/SWE-AF/pull/131) | Deterministic benchmark scorer (the evaluator their README already points at) — 8 tests, their full suite 1151 green | Open. **CLA pending**: bot can't map commit email `gd6505044@gmail.com` to a GitHub account → add email at github.com/settings/emails, sign CLA, click recheck | Operator signs CLA. Then watch — this repo's bots respond in minutes, so a real review is plausible |
