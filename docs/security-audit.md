...
| F19 | **The storefront paywall never ran.** `middleware.ts` priced every `POST /api/storefront/<template>/commission` in its x402 map, and `config.matcher` did not list that path — so Next never invoked the middleware there. A POST with a valid scope and **no `X-PAYMENT` header** returned `{"status":"commissioned"}`: a full escrowed pipeline, fronted from the prime's own wallet, for free. Contained only by every storefront being closed (§42) — a different bug — until one was opened. | E, B | **Exploited live** on the rehearsal deployment | Same, with real Circle USDC, once a mainnet storefront is opened |
| F27 | **The paywall priced every route in testnet money, on every deployment.** `middleware.ts` hardcoded `network: 'base-sepolia'` four times, justified as the payment rail being independent of `ONCHAIN_CHAIN`. True of a credit report, which is a data sale. False of the two routes that FUND ESCROW: on a Base mainnet deployment, `POST /api/jobs/external` and every storefront commission would answer a **faucet token that costs nothing to mint** with real Circle USDC locked in a real escrow. Every guard on those routes bounds how *much* — the daily cap, the fixed price, the margin test — and not one checked *what*. Compounding it, the $0.10 → $25 fixed bounty (F15) is a deliberate mUSDC subsidy whose own file header has said "on mainnet this becomes bounty pass-through" since it was written; pass-through was never built, so the subsidy was one env var away from being a 250x faucet on the house agent's wallet. | C | Audit, while checking what blocked a mainnet storefront | **This finding is entirely a mainnet delta** — on testnet the mismatch is invisible because both sides are already test money |
...
| MCP tool authorization | 28 tools. Every one that reads or writes user-owned data scopes on `auth.userId`, and the ones that read a row by id (`get_delegation_output`, `submit_work`) re-check ownership before acting; the rest expose public market data the guest board already shows. `requireAgent` returns 404 rather than 403 so it does not leak existence. Spot-checked, not exhaustively proven. No change. **2026-09-04: the manifest now has 67 tools, not 28 — the other 39 (office, office session, Repo Care, storefront, lineage) were never covered by this row.** Spot-checked three of the newer money-moving ones this pass: `withdraw_agent_eth` and `fund_agent_usdc` (`lib/mcp/handlers/office.ts`) both scope through `auth.userId` before touching a wallet, and `set_storefront`'s read path filters to `s.userId === auth.userId`. The other ~36 are unreviewed — this row's "spot-checked, not exhaustively proven" caveat now covers a much smaller fraction of the real surface than it did when it was written. |
