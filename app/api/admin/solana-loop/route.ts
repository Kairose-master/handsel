...
export async function POST(request: Request) {
  ...
  try {
    ...
    record('fund parties (SOL)', fundSig)

    const requesterAta = await getOrCreateAssociatedTokenAccount(connection, operator, mint, requester.publicKey)
    const workerAta = await getOrCreateAssociatedTokenAccount(connection, operator, mint, worker.publicKey)
    ...
    await mintTo(connection, operator, mint, workerAta.address, operator, Number(BOUNTY) * 4)
